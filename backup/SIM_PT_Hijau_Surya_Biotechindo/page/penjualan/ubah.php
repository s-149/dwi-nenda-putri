<?php
    ...
    $id=$_GET['id'];
    $sql = $koneksi->query("select * from penjualan where id_penjualan='$id'");
                        ...
                        <div class="col-md-6">
                                <table>
                                    ...
                                    <tr>
                                        <td>Nama Bibit</td>
                                        <td></td>
                                        <td><input type="text" class="form-control" name="nama_bibit" value="<?=$data['nama_bibit']?>" maxlength="12"  required autofocus /></td>
                                    </tr>
                                    <tr>
                                        <td>&nbsp;</td>
                                    </tr>
...
// mendefinisikan jumlah stok saat update data
    ...
    if ($simpan) {
        $sql = $koneksi->query("update  `penjualan` SET `id_penjualan`='$id_penjualan', `tanggal`='$tanggal', `jenis_penjualan`='$jenis_penjualan', `code_clone`='$code_clone', `nama_bibit`='$nama_bibit', `harga_satuan`='$harga_satuan',`jumlah`='$jumlah',`total_harga`='$total_harga',`tujuan`='$tujuan',`penerima`='$penerima',`ekspedisi`='$ekspedisi',`keterangan`='$keterangan' where id_penjualan='$id'");

        if ($sql) 
            {

                $sql1 = $koneksi->query("UPDATE `benih` SET `jumlah`='$_stok_benih' where code_clone='$code_clone' ");
