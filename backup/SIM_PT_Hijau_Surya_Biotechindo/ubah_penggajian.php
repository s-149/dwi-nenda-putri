...
if (empty($_SESSION['user']) AND empty($_SESSION['password']))
 ...
 if (isset($_POST['sementara'])) {
    ...
    $_sql = $koneksi->query("select * from penggajian where id_karyawan='$id_karyawan'");
    $_data = $_sql->fetch_assoc();
        ...
        // mendefinisikan data kontaminasi
            ...
            // mendefinisikan tujuan akhir intensip
                if ($_bagian2=='AdminLabolatorium') 
                        {
                            $_kontam=$total_kontam;
                            $_insentif1=$_insentif;
            ...
            $_gaji_kotor=$gaji_pokok+$tunjangan+$_insentif1;
                        ...
                        <div class="col-md-6">
                                <table>
                                    <tr>
                                        ...
                                        <td></td>
                                        <td>
                                            <input type="hidden" class="form-control" name="id_penggajian" value="<?=$id_penggajian?>"  required autofocus/>
                                            <input type="date" class="form-control" name="tanggal" value="<?=$_data['tanggal']?>"  required autofocus/>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>&nbsp;</td>
                        ...
                        <div class="col-md-6">
                                <table>
                                    <tr>
                                        ...
                                        <td>&nbsp;&nbsp;&nbsp;</td>
                                        <td width="80%"><label><?=BuatRp($_insentif1)?>.00</label>
                                            <input type="hidden" class="form-control" name="total_kontam" value="<?=$_kontam?>"  required autofocus />
                                            <input type="hidden" class="form-control" name="insentif" value="<?=$_insentif1?>"  required autofocus />
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>&nbsp;</td>
                                    ...
                                    <tr>
                                        ...
                                        <td>&nbsp;&nbsp;&nbsp;</td>
                                        <td width="80%">
                                            <textarea class="form-control" rows="5" name="keterangan"  required autofocus><?=$_data['keterangan']?></textarea>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>&nbsp;</td>
                                ...
                                <table style="float: right;" >
                                    <tr>
                                        <td width="80%">
                                            ...
                                            <a href="index.php?page=penggajian&aksi=ubah&id=<?=$id_penggajian?>" class="btn btn-primary">BATAL</a>
                                        </td>
                                    </tr>
                                </table>
                        </div>
 ...
 <?php }
  ...
  if (isset($_POST['simpan'])) {
    ...
    if ($simpan) {
        ...
        $sql = $koneksi->query("update  `penggajian` SET  `tanggal`='$tanggal', `id_karyawan`='$id_karyawan', `nama_karyawan`='$nama_karyawan', `gaji_pokok`='$gaji_pokok', `tunjangan`='$tunjangan',`bagian`='$bagian',`total_%_kontam`='$total_kontam',`isentif`='$insentif',`gaji_kotor`='$gaji_kotor',`id_absensi`='$id_absensi',`gaji_bersih`='$gaji_bersih',`keterangan`='$keterangan'  where id_penggajian='$id_penggajian'");

        $sql1 = $koneksi->query("update  `laporan_penggajian` SET  `tanggal`='$tanggal', `id_karyawan`='$id_karyawan', `nama_karyawan`='$nama_karyawan', `gaji_pokok`='$gaji_pokok', `tunjangan`='$tunjangan',`insentif`='$insentif',`gaji_kotor`='$gaji_kotor',`gaji_bersih`='$gaji_bersih'  where id_penggajian='$id_penggajian'");
        ...
        else
            {
                print'
                    ...
                    <script type="text/javascript">
                        ...
                        window.location.href="index.php?page=penggajian&aksi=ubah&id=<?=$id_penggajian?>";
