<?php
    ...
    $id=$_GET['id'];
    $sql = $koneksi->query("SELECT `id_bibit`, `tanggal_keluar`, `code_clone`, `nama_benih`, `no_indukan`, `jmlh_unit`, `keterangan` FROM `bibit` JOIN `benih` USING(`code_clone`)  where id_bibit='$id'");
...
?>
 ...
 <?php
    ...
    if ($simpan) {
        $sql = $koneksi->query("update  `bibit` SET `tanggal_keluar`='$tanggal_keluar', `code_clone`='$code_clone', `jmlh_unit`='$jmlh_unit', `keterangan`='$keterangan' where id_bibit='$id'");
